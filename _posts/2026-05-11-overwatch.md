...
MSSQL 내부를 탐색하던 중 `SQL07` 이라는 linked server가 존재하지만 정상적으로 연결되지 않는 것을 확인하고, 해당 호스트명을 attacker IP로 해석되도록 **DNS 레코드를 조작**한다. 
