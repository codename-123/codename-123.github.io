...
![로그인 페이지 탐색](/assets/htb-linux/yummy/export.png)
...
![로그인 페이지 탐색](/assets/htb-linux/yummy/jwt.png)

## Access Admin Dashboard

해당 Token을 브라우저의 쿠키에 삽입한 결과, `/admindashboard` 경로에 정상적으로 접근할 수 있었다.

![로그인 페이지 탐색](/assets/htb-linux/yummy/admindashboard.png)
...
위와 같이 `order_query` 값은 사용자 입력을 그대로 `f-string` 을 통해 SQL 쿼리에 삽입하고 있으며, `order_query`에 `desc; select ~` 같은 악의적인 값이 들어갈 경우 다중 쿼리 실행이나 테이블 삭제 등의 공격이 가능하다.
