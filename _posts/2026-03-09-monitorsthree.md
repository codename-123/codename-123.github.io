...
마지막으로 시스템에서 열린 포트를 조사하는 과정에서 **취약한 Duplicati 인스턴스**가 실행 중인 것을 확인한다. 공격자는 이 취약점을 악용해 `marcus` 사용자의 **SSH 공개 키를 루트 계정의 `authorized_keys`에 추가**하고, `marcus` 의 개인 키를 이용해 SSH로 root 계정에 접속하여 최종적으로 시스템의 **루트 권한을 탈취**하게 된다.
...
![MonitorsThree](/assets/htb-linux/monitorsthree/flag.png)